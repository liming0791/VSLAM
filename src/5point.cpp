...
bool Solve5PointEssential(std::vector<cv::Point2f> &pts1, std::vector<cv::Point2f> &pts2, std::vector<cv::Mat> &ret_E, std::vector<cv::Mat> &ret_P, std::vector<int> &ret_inliers, std::vector<cv::Mat> &inlierMats){
    ...
    for(size_t i=0; i < solutions.size(); i++) {
        ...
        cv::Mat best_E, best_P, best_inlierMat;
        int best_inliers = 0;
        bool found = false;

        for(int j=0; j < 4; j++) {
            pt3d = TriangulatePoint(cv::Point2d(pts1[0].x, pts1[0].y), cv::Point2d(pts2[0].x, pts2[0].y), P_ref, P[j]);
            double depth1 = CalcDepth(pt3d, P_ref);
            double depth2 = CalcDepth(pt3d, P[j]);
            cv::Mat inlierMat(num_pts, 1, CV_8UC1, cv::Scalar::all(0));
            ...
            if(depth1 > 0 && depth2 > 0){
                ...
                for(int k=1; k < num_pts; k++) {
                    pt3d = TriangulatePoint(cv::Point2d(pts1[k].x, pts1[k].y), cv::Point2d(pts2[k].x, pts2[k].y), P_ref, P[j]);
                ...
                if(inliers > best_inliers && inliers >= 5) {
                    ...
                    E.copyTo(best_E);
                    P[j].copyTo(best_P);
                    inlierMat.copyTo(best_inlierMat);
                    found = true;
                }
        ...
        if(found) {
            ...
            inlierMats.push_back(best_inlierMat);
        }
    }
